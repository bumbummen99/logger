...
class Logger {
  ...
  verbose(
    ...
    if (module in this.colors) {
      if (
        typeof this.colors[module] === 'string' &&
        (this.colors[module] as string) in chalk
      ) {
        colorFunction = chalk[
          this.colors[module] as Color as keyof ChalkInstance
        ] as ChalkInstance;
    ...
    if ((this.verboseness[module] || 1) >= verboseness) {
      console.log(
        `[${colorFunction(module)}][${verboseness}] ${message}`,
        ...extras
      );
    }
  }
